"""FastAPI backend with SSE streaming for the PAC1 dashboard — Claude Agent SDK."""
...
_cfg: ClaudeConfig | None = None


def _get_cfg() -> ClaudeConfig:
    global _cfg
    if _cfg is None:
        _cfg = ClaudeConfig.from_env()
    return _cfg
...
async def _run_single(
    cfg: ClaudeConfig, harness, run_id: str,
    ...
    try:
        ...
        telemetry = await run_task_claude(
            cfg, trial.harness_url, trial.instruction,
...
app = FastAPI(title="PAC1 Agent Dashboard API — Claude")
